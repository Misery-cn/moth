...
void CThread::start(bool detach) throw (sys::CException, sys::CSysCallException)
{
    before_start();
...
size_t CThread::get_stack_size() const throw (sys::CSysCallException)
{
    size_t stack_size = 0;
...
void CThread::join() throw (sys::CSysCallException)
{
    // 线程自己不能调用join
...
void CThread::detach() throw (sys::CSysCallException)
{
...
bool CThread::can_join() const throw (sys::CSysCallException)
{
    int state;
...
void CThread::stop(bool wait_stop) throw (sys::CException, sys::CSysCallException)
{
    if (!stop_)
    {
        stop_ = true;
